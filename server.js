...
app.post('/vote', (req, res) => {
  ...
  const vote = { q1: a1.trim(), q2: a2.trim(), q3: a3 ? a3.trim() : '', ts: new Date().toISOString() };
  votes.push(vote);
  saveData();
...
app.get('/export/current', (req, res) => {
  const rows = votes.map(v =>
    `"${v.ts}","${v.q1.replace(/"/g,'""')}","${v.q2.replace(/"/g,'""')}","${(v.q3||'').replace(/"/g,'""')}"`
  );
  const csv = ['Timestamp,Q1 - Qué sumarías a tu barrio,Q2 - En qué te capacitarías,Q3 - Qué te interesa más', ...rows].join('\n');
